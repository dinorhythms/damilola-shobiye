const Footer = () => {
  return (
    <footer className='bg-[#0C0C0C] mx-0 lg:mx-0'>
      <div className='container mx-auto text-center py-16 text-white'>
        <p className='font-bold text-sm'>IN MEMORY OF DAM DAM SHO</p>
        <p className='text-3xl my-4 font-bold'>
          This is the official memorial website created in memory of Barrister
          Mrs. Damilola O. Shobiye. {/* <br /> */}
          Our loving Wife, Mother, Sister, Daughter and Friend.
        </p>
        <p className='font-bold text-2xl'>(1982 -2024)</p>
      </div>
    </footer>
  );
};

export default Footer;
